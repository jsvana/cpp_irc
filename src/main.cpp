...
int main(int argc, char** argv) {
  ...
  // Example command
  bot.add_command_callback("test", [](Bot* bot, const Message& message, const Command& command) {
    if (command.args.size() < 1) {
      bot->channel_message(command.origin, *message.user + ": Must provide an argument");
      return;
    }
